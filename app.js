...
// --- SOLUCIÓN A LOS GHOST CLICKS DE LA RUEDA ---
function toggleFabMenu() {
    ...
    if (options.classList.contains('opacity-0')) { 
        ...
        options.classList.add('pointer-events-auto'); // Habilita clics
        icon.style.transform = 'rotate(45deg)'; 
    } else { 
        options.classList.add('opacity-0', 'translate-y-4'); 
        options.classList.remove('pointer-events-auto'); // Bloquea clics al cerrarse
...
async function changeDay(day, event) {
    ...
    try {
        ...
        exercises.forEach(ex => {
            ...
            // Re-inicializar variables de timer
...
async function saveToCloud(exId, totalSets, exName, exType, btnEvent) {
    ...
    // Obtenemos tiempo de ejercicio y luego lo reseteamos visualmente
...
async function loadEvolucion(exId, exName, exType, forceReload = false) {
    ...
    try { 
        ...
        const chartDataDuration = dates.map(d => (groupedData[d].duration / 60).toFixed(2)); // Tiempo en Minutos con decimales
        ...
        <div>
            ...
            <div class="overflow-x-auto rounded-xl border border-custom-border bg-[#0a0a0a]"><table class="w-full text-left border-collapse"><thead class="bg-[#171717]"><tr class="border-b border-custom-border text-custom-textMuted text-[10px] uppercase tracking-widest"><th class="py-3 px-3 font-bold">Día / Tiempo</th><th class="py-3 px-2 font-bold">Series</th><th class="py-3 px-3 font-bold text-right">Acción</th></tr></thead><tbody>${tableRows}</tbody></table></div>
        </div>`; 
        container.classList.remove('hidden'); 

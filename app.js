...
function timerDragMove(e) {
    ...
    miniTimer.style.bottom = 'auto'; miniTimer.style.right = 'auto'; miniTimer.style.transform = 'none';
}
function timerDragEnd(e) {
    ...
    if (rect.left + rect.width / 2 < screenWidth / 2) { miniTimer.style.left = '16px'; } 
    else { miniTimer.style.left = `${screenWidth - rect.width - 16}px`; }
...
function typeWriter() {
    ...
    if(!el) return;
    if (typeIndex < subtitleText.length) { el.innerHTML += subtitleText.charAt(typeIndex); typeIndex++; setTimeout(typeWriter, 20); } 
...
function toggleFabMenu() {
    ...
    if (options.classList.contains('opacity-0')) { 
        options.classList.remove('opacity-0', 'translate-y-4', 'pointer-events-none'); icon.style.transform = 'rotate(45deg)'; 
    } else { 
        options.classList.add('opacity-0', 'translate-y-4', 'pointer-events-none'); icon.style.transform = 'rotate(0deg)'; 
    }
}
...
window.toggleExTimer = function(exId) {
    ...
    // Íconos SVG Estilo HAT
    const playIcon = `<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg> Iniciar`;
    const pauseIcon = `<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M6 4h4v16H6zm8 0h4v16h-4z"/></svg> Pausar`;
...
window.resetExTimer = function(exId) {
    ...
    const btn = document.getElementById(`btn-extimer-${exId}`);
    btn.innerHTML = `<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg> Iniciar`;
    btn.classList.replace('text-green-400', 'text-orange-400');
};
...
function openTimerModal() {
    ...
    if (timerSecondsLeft === 0) { 
        document.getElementById('timer-min').value = ''; document.getElementById('timer-seg').value = ''; 
        document.getElementById('timer-inputs').classList.remove('hidden'); document.getElementById('timer-display').classList.add('hidden'); 
    } else { 
...
function toggleTimer() {
    ...
    if (timerInterval) {
        ...
        timerInterval = setInterval(() => {
            ...
            if(timerSecondsLeft <= 10 && timerSecondsLeft > 0) { playTap(); } // Sonido avisando los últimos 10 seg
...
function hideMiniTimer(stopAsWell = false) { 
    const miniWidget = document.getElementById('mini-timer-widget'); miniWidget.classList.add('translate-x-[-150%]', 'opacity-0'); miniWidget.classList.remove('translate-x-0', 'opacity-100'); 
    if (stopAsWell) stopTimer(); 
}

function updateTimerDisplay() { 
    let m = Math.floor(timerSecondsLeft / 60).toString().padStart(2, '0'); let s = (timerSecondsLeft % 60).toString().padStart(2, '0'); let txt = `${m}:${s}`; 
    document.getElementById('timer-countdown').innerText = txt; document.getElementById('mini-timer-countdown').innerText = txt; 
}


// --- RESTO DEL CÓDIGO (Auth, Guardados, UI) ---
...
function animateTokenLoss() {
    ...
    animEl.style.left = `${rect.left + (rect.width / 2) - 10}px`; animEl.style.top = `${rect.top}px`; document.body.appendChild(animEl);
...
function promptDeleteSavedRoutine(savedId) {
    ...
    document.getElementById('btn-confirm-delete-saved-action').onclick = async () => {
        ...
        try { await supabaseClient.from('saved_routines').delete().eq('id', pendingSavedRoutineId); openRoutinesModal(); } 
        catch(e) { showToast("Error: " + e.message); } 
        finally { btn.innerText = "Sí, Borrar"; btn.disabled = false; }
    };
}
    ...
    try {
        ...
        const exercisesToInsert = routineData.map((ex, index) => { 
            ...
            if (globalMedia) {
                ...
                if (match) { imgUrl = match.image_url || ""; hasImg = match.has_image || false; ytUrl = match.youtube_url || ""; hasVid = match.has_video || false; }
            }
...
async function changeDay(day, event) {
    ...
    try {
        ...
        exercises.forEach(ex => {
            ...
            // Iconos limpios SVG en lugar de Emojis
            let tIcon = window.exerciseTimers[safeId].interval 
            ...
            <div class="bg-custom-card p-6 rounded-3xl border border-custom-border shadow-xl flex flex-col relative group" data-ex-id="${safeId}">
                ...
                <div class="flex flex-col items-center justify-center mt-4 mb-2 bg-[#0a0a0a] rounded-xl p-3 border border-[#262626]">
                    ...
                    <div class="flex items-center justify-center gap-3 w-full">
                        <span class="text-2xl font-black text-white tracking-widest w-24 text-center" id="display-extimer-${safeId}">${tDisplay}</span>
                        <div class="flex gap-2">
                            <button onclick="toggleExTimer('${safeId}')" id="btn-extimer-${safeId}" class="flex items-center justify-center w-10 h-10 bg-[#171717] border border-[#262626] rounded-lg ${tColor} hover:bg-[#262626] transition-colors" title="Play/Pausa">
                                ${tIcon}
                            </button>
                            <button onclick="resetExTimer('${safeId}')" class="flex items-center justify-center w-10 h-10 bg-[#171717] border border-[#262626] rounded-lg text-red-500 hover:bg-red-500/10 transition-colors" title="Limpiar y Reiniciar a Cero">
...
async function saveToCloud(exId, totalSets, exName, exType, btnEvent) {
    ...
    if(window.exerciseTimers[exId]) {
        if(window.exerciseTimers[exId].interval) { 
            ...
            document.getElementById(`btn-extimer-${exId}`).innerHTML = '<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>'; 
    ...
    try { 
        ...
        btn.classList.add('bg-red-600'); btnText.innerText = "ERROR"; 

...
function openModal(modalId) {
    ...
    const targetModal = document.getElementById(modalId); if(targetModal) targetModal.classList.remove('hidden'); 
...
function closeAllModals() {
    ...
    if (timerSecondsLeft > 0) { const miniWidget = document.getElementById('mini-timer-widget'); if(miniWidget) { miniWidget.classList.remove('translate-x-[-150%]', 'opacity-0', 'hidden'); miniWidget.classList.add('translate-x-0', 'opacity-100'); } }
}
...
function toggleFabMenu() {
    ...
    if (options.classList.contains('opacity-0')) { 
        options.classList.remove('opacity-0', 'translate-y-4'); options.classList.add('pointer-events-auto'); icon.style.transform = 'rotate(45deg)'; 
    } else { 
        options.classList.add('opacity-0', 'translate-y-4'); options.classList.remove('pointer-events-auto'); icon.style.transform = 'rotate(0deg)'; 
    }
}
...
function resetExTimer(exId) {
    ...
    document.getElementById(`display-extimer-${exId}`).innerText = "0s"; // Solucionado a 0s
    ...
    if (timerInterval) {
        ...
        timerInterval = setInterval(() => {
            ...
            if(timerSecondsLeft <= 0) { 
                ...
                // Seguridad si falta el modal
                if(document.getElementById('modal-timer-finished')) { openModal('modal-timer-finished'); } 
                else { closeAllModals(); showToast("¡Descanso Terminado!"); }
            } 
...
function resumeRestTimerIfActive() {
    ...
    if (savedEnd) {
        ...
        if (remaining > 0) {
            ...
            localStorage.removeItem('hat_rest_timer_end'); 
            if(document.getElementById('modal-timer-finished')) { openModal('modal-timer-finished'); }
        }
    }
}
        ...
        exercises.forEach(ex => {
            ...
            <div class="bg-custom-card p-6 rounded-3xl border border-custom-border shadow-xl flex flex-col relative group" data-ex-id="${safeId}">
                ...
                <div class="flex flex-col items-center justify-center mt-4 mb-2 bg-[#0a0a0a] rounded-xl p-3 border border-[#262626]">
                    ...
                    <div class="flex items-center justify-center gap-4 w-full">
                        ...
                        <div class="flex gap-2">
                            <button onclick="resetExTimer('${safeId}')" class="flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-red-500 hover:bg-[#262626] transition-colors" title="Limpiar y Reiniciar a Cero">
                                ${SVG_RESET}
                            </button>
                            <button onclick="toggleExTimer('${safeId}')" id="btn-extimer-${safeId}" class="flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl ${tColorClass} hover:bg-[#262626] transition-colors" title="Play/Pausa">
                                ${tIcon}
                            </button>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                    <button onclick="saveToCloud('${safeId}', ${ex.sets}, '${exNameForJS}', '${exType}', event)" class="w-full bg-custom-primary py-4 rounded-xl font-extrabold text-white tracking-widest hover:bg-custom-hover transition-all active:scale-95 shadow-lg relative overflow-hidden text-sm">
                        <span class="relative z-10 btn-text">GUARDAR SESIÓN</span>
                    </button>
                    <button onclick="loadEvolucion('${safeId}', '${exNameForJS}', '${exType}')" id="btn-evo-${safeId}" class="w-full bg-transparent border-2 border-custom-border text-custom-textMuted py-4 rounded-xl font-bold hover:border-custom-primary hover:text-white transition-all active:scale-95 text-sm uppercase tracking-widest">
                        VER PROGRESO
                    </button>
                </div>
...
function extractYoutubeId(url) { 
    if(!url) return null; const regExp = /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/; 
...
async function saveToCloud(exId, totalSets, exName, exType, btnEvent) {
    ...
    // RECOLECTAR SERIES PRIMERO (sin resetear nada todavía)
    for(let i=1; i<=totalSets; i++) { 
        ...
        if (checked) {
            if (exType === 'tiempo') {
                ...
                if(totalSecs > 0) { logs.push({ user_id: currentUserId, exercise_name: exName, weight: 0, reps: 0, time_seconds: totalSecs, exercise_type: 'tiempo', exercise_duration: 0, set_number: i, log_date: dateString }); }
            } else {
                let w = document.getElementById(`peso-${exId}-${i}`).value; let r = document.getElementById(`reps-${exId}-${i}`).value;
                if(w && r) { logs.push({ user_id: currentUserId, exercise_name: exName, weight: parseFloat(w), reps: parseInt(r), time_seconds: 0, exercise_type: 'carga', exercise_duration: 0, set_number: i, log_date: dateString }); }
            }
        } 
    }
    
    // SI NO HAY SERIES MARCADAS, AVISAR Y CANCELAR (sin resetear el tiempo)
    if(logs.length === 0) { 
    ...
    // SI HAY SERIES, RECIÉN AHORA OBTENEMOS EL TIEMPO Y LO RESETEAMOS
    let exTotalSeconds = getExCurrentSeconds(exId);
    resetExTimer(exId); 
    
    // Actualizamos las logs con el tiempo final
    logs.forEach(log => log.exercise_duration = exTotalSeconds);
...
// =========================================================================
// EXPORTACIÓN GLOBAL DE TODAS LAS FUNCIONES (PREVIENE FALLOS DEL HTML)
// =========================================================================

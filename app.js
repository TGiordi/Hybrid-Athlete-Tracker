...
// Iniciar el contexto de audio al primer clic o toque
...
function playTap() { 
    playTone(1200, 'sine', 0.05, 0.015); 
} 

function playPop() { 
    playTone(900, 'triangle', 0.08, 0.02); 
} 
...
function timerDragEnd(e) {
    ...
    // Iman magnético hacia la izquierda o derecha
    ...
    // Evitar que se salga por arriba o por abajo
...
function openModal(modalId) {
    ...
    // Limpieza al abrir ciertos modales
...
function closeAllModals() {
    ...
    // Si cerramos y el cronómetro de descanso está activo, mostramos el mini widget
...
function closeFabAndRun(callback) { 
    toggleFabMenu(); 
    callback(); 
}
...
window.startGlobalWorkout = function() {
    ...
    // Guardamos la hora de inicio real en la PC/Celular
...
// Recuperar entrenamiento si se recargó la página
...
window.toggleExTimer = function(exId) {
    ...
    if(t.interval) {
        ...
        t.interval = null;
        btn.innerHTML = '<svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg> Retomar';
        btn.classList.replace('text-green-400', 'text-orange-400');
    } else {
        // Iniciar
        btn.innerHTML = '<svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M6 4h4v16H6zm8 0h4v16h-4z"/></svg> Pausar';
        ...
        t.interval = setInterval(() => {
            t.seconds++;
            let m = Math.floor(t.seconds / 60).toString().padStart(2, '0'); 
            let s = (t.seconds % 60).toString().padStart(2, '0');
...
window.resetExTimer = function(exId) {
    ...
    window.exerciseTimers[exId] = { seconds: 0, interval: null };
    
    document.getElementById(`display-extimer-${exId}`).innerText = "00:00";
    const btn = document.getElementById(`btn-extimer-${exId}`);
    btn.innerHTML = '<svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg> Iniciar';
    btn.classList.replace('text-orange-400', 'text-green-400');
};
...
function toggleTimer() {
    ...
    if (timerInterval) {
        ...
        timerInterval = setInterval(() => {
            ...
            // SONIDO EN LOS ÚLTIMOS 10 SEGUNDOS
...
async function submitSupportTicket() {
    ...
    try {
        ...
        const feedback = document.getElementById('support-msg-feedback');
        feedback.innerText = "¡Recibimos tu mensaje! Lo leeremos pronto."; 
        feedback.classList.remove('hidden');
        
        setTimeout(() => { 
            closeAllModals(); 
            setTimeout(() => { 
                document.getElementById('support-text').classList.remove('hidden'); 
...
function updateAuthUI() { 
    ...
    if(isSignUp) { 
        modal.classList.replace('bg-custom-card', 'bg-custom-primary'); 
        modal.classList.replace('border-custom-border', 'border-[#d43e20]'); 
        title.innerText = "Crear Nueva Cuenta"; 
        inputs.forEach(inp => { 
            inp.className = "w-full bg-[#171717] border border-[#262626] p-3 pr-12 rounded-xl outline-none focus:border-white text-white placeholder-white/50 transition-all"; 
        }); 
        eyeBtn.className = "absolute inset-y-0 right-0 px-4 flex items-center text-white/50 hover:text-white transition-colors cursor-pointer"; 
        btn.innerText = "Registrarme"; 
        btn.className = "w-full bg-[#171717] text-white py-3 rounded-xl font-bold hover:bg-black border border-black/50 transition-colors shadow-lg"; 
        toggleMsg.innerHTML = "O <span onclick='toggleAuthMode()' class='cursor-pointer font-extrabold underline hover:text-black transition-colors'>ingresá a tu cuenta acá</span>"; 
        toggleMsg.className = "text-center text-sm text-white mt-4 transition-colors font-medium"; 
        closeBtn.className = "mt-8 w-full text-[10px] text-white/80 uppercase tracking-[0.3em] font-bold hover:text-white transition-colors"; 
        forgotPass.classList.add('hidden'); 
    } else { 
        modal.classList.replace('bg-custom-primary', 'bg-custom-card'); 
        modal.classList.replace('border-[#d43e20]', 'border-custom-border'); 
        title.innerText = "Bienvenido Atleta"; 
        inputs.forEach(inp => { 
            inp.className = "w-full bg-custom-bg border border-custom-border p-3 pr-12 rounded-xl outline-none focus:border-custom-primary text-white transition-all"; 
        }); 
        eyeBtn.className = "absolute inset-y-0 right-0 px-4 flex items-center text-custom-textMuted hover:text-custom-primary transition-colors cursor-pointer"; 
        btn.innerText = "Iniciar Sesión"; 
        btn.className = "w-full bg-custom-primary text-white py-3 rounded-xl font-bold hover:bg-custom-hover transition-colors shadow-lg"; 
        toggleMsg.innerHTML = "¿No tenés cuenta? <br><span onclick='toggleAuthMode()' class='text-custom-primary cursor-pointer font-bold underline inline-block mt-1'>Registrate gratis acá</span>"; 
        toggleMsg.className = "text-center text-sm text-custom-textMuted mt-4 transition-colors font-medium"; 
        closeBtn.className = "mt-8 w-full text-[10px] text-custom-textMuted uppercase tracking-[0.3em] font-bold hover:text-white transition-colors"; 
...
function showMessage(text, isError = true) { 
    ...
    msgBox.classList.remove('hidden', 'bg-red-500/10', 'text-red-500', 'text-red-400', 'bg-green-500/10', 'text-green-500', 'text-green-400', 'bg-[#171717]', 'border-[#262626]'); 
    
    if(isError) { 
        if(isSignUp) msgBox.classList.add('bg-[#171717]', 'text-red-400', 'border-[#262626]'); 
        else msgBox.classList.add('bg-red-500/10', 'text-red-500', 'border-red-500/20'); 
    } else { 
        if(isSignUp) msgBox.classList.add('bg-[#171717]', 'text-green-400', 'border-[#262626]'); 
...
async function handleAuth() {
    ...
    if(!email || !password) return showMessage("Completá tu email y contraseña."); 
    ...
    try {
        if (isSignUp) { 
            ...
            if (error) throw error; 
            showMessage("¡Cuenta creada! Ya podés iniciar sesión.", false); 
        } else { 
            const { data, error } = await supabaseClient.auth.signInWithPassword({ email, password }); 
            if (error) throw error; 
            currentUserId = data.user.id; 
            loadDashboardView(data.user.email); 
        }
    } catch (err) { 
        let errorTxt = err.message; 
        if (errorTxt.includes("Invalid login credentials")) errorTxt = "Email o contraseña incorrectos."; 
        if (errorTxt.includes("already registered")) errorTxt = "Este email ya tiene cuenta."; 
        showMessage(errorTxt, true); 
    } finally { 
        btn.innerText = isSignUp ? "Registrarme" : "Iniciar Sesión"; 
        btn.disabled = false; 
    }
}
...
async function handleResetPassword() {
    const email = document.getElementById('auth-email').value; 
    if(!email) return showMessage("Ingresá tu email arriba y presioná este botón.", true);
    ...
    try { 
        ...
        if (error) throw error; 
        showMessage("Si el email está registrado, te llegará un link para cambiar la contraseña.", false);
    } catch (err) { 
...
async function saveNewPassword() {
    ...
    try {
        ...
        if (errStr.includes("different from the old password") || errStr.includes("different from the original")) { 
            ...
            msgBox.innerText = "El link ha caducado o hubo un error. Volvé a pedir el correo."; 
        }
...
async function updateCreditsDisplay() {
    try { 
        const { data, error } = await supabaseClient.from('profiles').select('ai_credits, has_infinite_credits').eq('id', currentUserId).single();
        if(data) { 
            document.getElementById('ai-credit-count').innerHTML = data.has_infinite_credits ? '<span class="text-lg leading-none flex items-center justify-center translate-y-[1px]">&infin;</span>' : data.ai_credits; 
        } else { 
            document.getElementById('ai-credit-count').innerText = "10"; 
        }
    } catch(e) { 
        document.getElementById('ai-credit-count').innerText = "10"; 
    }
...
async function openRoutinesModal() {
    ...
    try {
        ...
        if(error) throw error;
        if(data.length === 0) { 
            list.innerHTML = '<div class="text-center text-xs text-custom-textMuted py-4">No tenés rutinas guardadas.</div>'; 
            return; 
        }
        ...
        list.innerHTML = `<div class="text-xs text-red-500">Error: ${e.message}</div>`; 
    }
}
...
function handleAIGenerationRequest() { 
...
async function processAIPrompt() {
    ...
    const msgBox = document.getElementById('ai-error-msg');
    ...
    try { 
        const { count, error } = await supabaseClient.from('user_routines').select('*', { count: 'exact', head: true }).eq('user_id', currentUserId);
        if (error) throw error; 
        ...
        if (count > 0) openModal('modal-confirm-ai-overwrite'); 
        else proceedWithAIGeneration();
    } catch(e) { 
...
async function proceedWithAIGeneration() {
    closeAllModals(); 
    document.getElementById('loading-title').innerText = "Diseñando Rutina..."; 
    document.getElementById('loading-desc').innerText = "El Coach Inteligente está diseñando tu semana de entrenamiento. Por favor, no cierres esta ventana."; 
    ...
    try {
        ...
        const { error: dbError } = await supabaseClient.from('user_routines').insert(exercisesToInsert); 
        ...
        openModal('modal-ai-success');
    } catch(e) { 
        document.getElementById('ai-loading-overlay').classList.add('hidden'); 
        document.getElementById('ai-loading-overlay').classList.remove('flex'); 
        openModal('modal-ai-coach'); 
        document.getElementById('ai-error-msg').innerText = "Detalle: " + e.message; 
        document.getElementById('ai-error-msg').classList.remove('hidden'); 
    }
}

function openChatModal() { 
    const savedChat = localStorage.getItem(`hat_chat_${currentUserId}`); 
    if(savedChat) { window.chatHistory = JSON.parse(savedChat); } 
    else { window.chatHistory = []; } 
    renderChat(); 
...
function renderChat() {
    ...
    container.innerHTML = '<div class="text-center text-xs font-bold text-custom-textMuted py-4 uppercase tracking-widest">El historial se guarda en tu dispositivo</div><div class="chat-bubble-ai">¡Hola! Soy tu Coach Inteligente. ¿En qué te ayudo hoy con tu entrenamiento?</div>';
    
    window.chatHistory.forEach(msg => { 
        const isUser = msg.role === 'user'; 
        const html = `<div class="${isUser ? 'chat-bubble-user' : 'chat-bubble-ai'}">${formatMarkdown(escapeHTML(msg.parts[0].text))}</div>`; 
...
async function sendChatMessage() {
    ...
    input.value = ''; input.style.height = ''; renderChat();
    ...
    try {
        ...
        const aiReply = data.response.candidates[0].content.parts[0].text; 
        window.chatHistory.push({ role: 'model', parts: [{ text: aiReply }] }); 
        localStorage.setItem(`hat_chat_${currentUserId}`, JSON.stringify(window.chatHistory));
        ...
        window.chatHistory.push({ role: 'model', parts: [{ text: `❌ Error: ${e.message}` }] }); 
    } finally { 
        renderChat(); btn.disabled = false; 
        btn.innerHTML = '<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"></path></svg>'; 
    }
}

// --- RENDERIZADO Y CONTROL DE EJERCICIOS ---
...
async function confirmDeleteEntireRoutine() { 
    ...
    try { 
        ...
        alert("Error al borrar rutina: " + e.message); 
    } finally { 
        document.getElementById('ai-loading-overlay').classList.add('hidden'); 
        document.getElementById('ai-loading-overlay').classList.remove('flex'); 
    } 
...
function toggleMediaInput(type) { 
    ...
    else { container.classList.add('hidden'); document.getElementById(`new-ex-${type}`).value = ''; } 
}
...
function openEditExerciseModal(exId) { 
    ...
    document.getElementById('new-ex-video').value = ex.has_video ? `https://youtu.be/${ex.youtube_url}` : ''; 
...
function promptDeleteExercise(exId, exName) { 
    ...
    btn.onclick = async () => { 
        ...
        try { 
            ...
            btn.innerText = "Sí, quitar"; btn.disabled = false; 
        } 
    }; 
}
...
async function saveExercise() {
    ...
    try { 
        ...
        msgBox.innerText = "Error: " + err.message; msgBox.classList.remove('hidden'); 
    } finally { 
        btn.innerText = currentEditExerciseId ? "ACTUALIZAR EJERCICIO" : "GUARDAR EJERCICIO"; btn.disabled = false; 
    }
}
...
async function changeDay(day, event) {
    ...
    if(event) { 
        playTap(); 
        document.querySelectorAll('.tab-btn').forEach(b => { b.classList.remove('active'); b.classList.add('text-custom-textMuted'); b.classList.remove('text-white'); }); 
        event.target.classList.add('active'); event.target.classList.remove('text-custom-textMuted'); event.target.classList.add('text-white'); 
        ...
        document.querySelectorAll('.tab-btn').forEach(b => { 
            if(b.innerText.toLowerCase() === day.toLowerCase()) { 
                b.classList.add('active'); b.classList.remove('text-custom-textMuted'); b.classList.add('text-white'); 
                b.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' }); 
            } else { 
                b.classList.remove('active'); b.classList.add('text-custom-textMuted'); b.classList.remove('text-white'); 
            } 
        }); 
    }
    ...
    try {
        ...
        exercises.forEach(ex => {
            ...
            let tIcon = window.exerciseTimers[safeId].interval ? '⏸️ Pausar' : '▶️ Iniciar';
            ...
            <div class="bg-custom-card p-6 rounded-3xl border border-custom-border shadow-xl flex flex-col relative group" data-ex-id="${safeId}">
                ...
                <div class="flex items-center justify-between mt-2 mb-2 px-1 bg-[#0a0a0a] rounded-lg p-2 border border-[#262626]">
                    <span class="text-xs text-white font-bold tracking-widest uppercase">Tiempo</span>
                    <div class="flex items-center gap-2">
                        <span class="text-sm font-black text-white w-12 text-center" id="display-extimer-${safeId}">${tDisplay}</span>
                        <button onclick="resetExTimer('${safeId}')" class="px-2 py-1 bg-[#171717] rounded-md font-bold text-xs text-red-400 hover:text-red-300 transition-colors border border-[#262626]">⏹️</button>
                        <button onclick="toggleExTimer('${safeId}')" id="btn-extimer-${safeId}" class="px-3 py-1 bg-[#171717] rounded-md font-bold text-xs ${tColor} transition-colors border border-[#262626]">${tIcon}</button>
                    </div>
                </div>
...
// --- GUARDAR Y CARGAR HISTORIAL (EVOLUCIÓN) ---
async function saveToCloud(exId, totalSets, exName, exType, btnEvent) {
    ...
    // Frenamos el timer si estaba corriendo
    let exTotalSeconds = 0;
    if(window.exerciseTimers[exId]) {
        if(window.exerciseTimers[exId].interval) { 
            clearInterval(window.exerciseTimers[exId].interval); 
            window.exerciseTimers[exId].interval = null; 
            document.getElementById(`btn-extimer-${exId}`).innerHTML = '<svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg> Retomar'; 
            document.getElementById(`btn-extimer-${exId}`).classList.replace('text-green-400', 'text-orange-400');
        }
    ...
    try { 
        ...
        if(!evoContainer.classList.contains('hidden')) {
            loadEvolucion(exId, exName, exType, true); 
        }
...
async function loadEvolucion(exId, exName, exType, forceReload = false) {
    ...
    try { 
        ...
        const chartDataDuration = dates.map(d => Math.round(groupedData[d].duration / 60)); // Tiempo de Ejercicio en Minutos
...
function promptDeleteLog(exName, rawDate, exId, exType) {
    ...
    btn.onclick = async () => { 
        ...
        try { 
            ...
            closeAllModals(); 
            loadEvolucion(safeExId, safeExName, exType, true); 
        } catch(e) { 
            msgBox.innerText = e.message; 
            msgBox.classList.remove('hidden'); 
        } finally { 
...
function promptEditLog(exId, exName, dateStr, exType) {
    ...
    dayData.sets.forEach(s => { 
        if(exType === 'tiempo') {
            let m = Math.floor(s.time_seconds / 60); 
            let seg = s.time_seconds % 60;
            let padM = m.toString().padStart(2, '0'); 
            let padSeg = seg.toString().padStart(2, '0');
    ...
    btn.onclick = async () => { 
        ...
        try { 
            for(let s of dayData.sets) { 
                if(exType === 'tiempo') {
                    let m = document.getElementById(`edit-m-${s.id}`).value || 0; 
                    let seg = document.getElementById(`edit-s-${s.id}`).value || 0;
                    let t = (parseInt(m)*60) + parseInt(seg);
                    await supabaseClient.from('workout_logs').update({ time_seconds: t }).eq('id', s.id);
                } else {
                    let w = document.getElementById(`edit-w-${s.id}`).value; 
                    let r = document.getElementById(`edit-r-${s.id}`).value; 

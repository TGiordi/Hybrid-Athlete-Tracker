...
// ÍCONOS SVG PUROS PARA EL CRONÓMETRO (Estilo HAT - Mismo tamaño)
window.SVG_PLAY = `<svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>`;
window.SVG_PAUSE = `<svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M6 5h4v14H6zm8 0h4v14h-4z"/></svg>`;
...
window.playTap = function() { playTone(1200, 'sine', 0.05, 0.015); };
window.playPop = function() { playTone(900, 'triangle', 0.08, 0.02); };
...
function formatTime(totalSeconds) {
    ...
    if(h > 0) return `${h}h ${m}m`; if(m > 0 && s > 0) return `${m}m ${s}s`; if(m > 0) return `${m}m`; return `${s}s`;
}
...
window.openModal = function(modalId) {
    ...
    if(modalId === 'modal-auth') { isSignUp = false; window.updateAuthUI(); document.getElementById('auth-password').type = 'password'; }
...
window.toggleFabMenu = function() {
    ...
    if (options.classList.contains('opacity-0')) { 
        ...
        options.classList.add('pointer-events-auto'); 
        icon.style.transform = 'rotate(45deg)'; 
    } else { 
        options.classList.add('opacity-0', 'translate-y-4', 'pointer-events-none'); 
        options.classList.remove('pointer-events-auto'); 
        icon.style.transform = 'rotate(0deg)'; 
    }
};

window.closeFabAndRun = function(callbackFunction) { 
...
document.addEventListener('click', (e) => {
    ...
    if (fabContainer && !e.target.closest('#fab-container') && !fabOptions.classList.contains('opacity-0')) { window.toggleFabMenu(); }
});
document.getElementById('modal-overlay').addEventListener('click', function(e) { if (e.target === this) window.closeAllModals(); });
...
document.addEventListener('DOMContentLoaded', async () => {
    setTimeout(() => { const el = document.getElementById("typewriter-text"); if(!el) return; if (typeIndex < subtitleText.length) { el.innerHTML += subtitleText.charAt(typeIndex); typeIndex++; setTimeout(arguments.callee, 20); } else { el.classList.remove('typewriter-cursor'); } }, 500);
    ...
    ['new-ex-name', 'new-ex-sets', 'new-ex-reps'].forEach(id => { document.getElementById(id)?.addEventListener('keypress', e => { if(e.key === 'Enter') window.saveExercise(); }); });

    try {
        if (window.supabase) {
            supabaseClient = window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
            let isRecovering = window.location.hash.includes('type=recovery');
            if (isRecovering) { window.openModal('modal-new-pwd'); } 
            else { const { data: { session } } = await supabaseClient.auth.getSession(); if(session) { currentUserId = session.user.id; window.loadDashboardView(session.user.email); } }
            supabaseClient.auth.onAuthStateChange((event, session) => {
                if (event === 'PASSWORD_RECOVERY') { isRecovering = true; window.openModal('modal-new-pwd'); } 
                else if (event === 'SIGNED_IN' && session) { setTimeout(() => { if(!isRecovering && !currentUserId) { currentUserId = session.user.id; window.loadDashboardView(session.user.email); } }, 500); }
...
window.startGlobalWorkout = function() {
    window.playVictory(); document.getElementById('btn-start-workout').classList.add('hidden'); document.getElementById('global-workout-timer').classList.remove('hidden');
    ...
    globalTimerInterval = setInterval(() => { const startTime = parseInt(localStorage.getItem('hat_workout_start') || Date.now()); globalSeconds = Math.floor((Date.now() - startTime) / 1000); if (globalSeconds >= MAX_TIMER_SECONDS) { globalSeconds = MAX_TIMER_SECONDS; window.showToast("⚠️ Límite de 24h alcanzado."); } window.updateGlobalTimerDisplay(); }, 1000);
};
...
// Doble enlace por si el HTML usa uno u otro nombre
window.promptStopGlobalWorkout = function() { window.openModal('modal-confirm-stop-workout'); };
window.stopGlobalWorkout = function() { window.openModal('modal-confirm-stop-workout'); };
...
window.confirmStopGlobalWorkout = async function() {
    ...
    try { await supabaseClient.from('workout_sessions').insert([{ user_id: currentUserId, session_date: dateString, duration_seconds: globalSeconds }]); window.closeAllModals(); window.showToast(`¡Entrenamiento finalizado! Tiempo: ${formatTime(globalSeconds)}`); window.playVictory(); } catch(e) { console.error("Error guardando sesión global:", e); }
...
window.resumeGlobalWorkoutIfActive = function() {
    ...
    if (savedStart) {
        ...
        document.getElementById('btn-start-workout').classList.add('hidden'); document.getElementById('global-workout-timer').classList.remove('hidden'); window.updateGlobalTimerDisplay();
        globalTimerInterval = setInterval(() => { const startTime = parseInt(localStorage.getItem('hat_workout_start') || Date.now()); globalSeconds = Math.floor((Date.now() - startTime) / 1000); if (globalSeconds >= MAX_TIMER_SECONDS) globalSeconds = MAX_TIMER_SECONDS; window.updateGlobalTimerDisplay(); }, 1000);
    }
};

// --- GESTOR LOCAL DE TIEMPOS DE EJERCICIO ---
window.getExTimerState = function(exId) { let timers = JSON.parse(localStorage.getItem('hat_ex_timers') || '{}'); return timers[exId] || { running: false, acc: 0, lastStart: 0 }; };
window.saveExTimerState = function(exId, state) { let timers = JSON.parse(localStorage.getItem('hat_ex_timers') || '{}'); timers[exId] = state; localStorage.setItem('hat_ex_timers', JSON.stringify(timers)); };
window.clearExTimerState = function(exId) { let timers = JSON.parse(localStorage.getItem('hat_ex_timers') || '{}'); delete timers[exId]; localStorage.setItem('hat_ex_timers', JSON.stringify(timers)); };
window.getExCurrentSeconds = function(exId) { let state = window.getExTimerState(exId); let secs = state.acc; if (state.running) { secs += Math.floor((Date.now() - state.lastStart) / 1000); } if (secs > MAX_TIMER_SECONDS) secs = MAX_TIMER_SECONDS; return secs; };
...
window.toggleExTimer = function(exId) {
    ...
    let state = window.getExTimerState(exId);
    
    if(state.running) {
        if(t.interval) clearInterval(t.interval); t.interval = null;
        state.acc += Math.floor((Date.now() - state.lastStart) / 1000); state.running = false; window.saveExTimerState(exId, state);
        btn.innerHTML = window.SVG_PLAY; btn.className = `flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-green-500 hover:bg-[#262626] transition-colors`;
    } else {
        state.running = true; state.lastStart = Date.now(); window.saveExTimerState(exId, state);
        btn.innerHTML = window.SVG_PAUSE; btn.className = `flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-orange-500 hover:bg-[#262626] transition-colors`;
        t.interval = setInterval(() => { let currentSecs = window.getExCurrentSeconds(exId); if (currentSecs >= MAX_TIMER_SECONDS) currentSecs = MAX_TIMER_SECONDS; let m = Math.floor(currentSecs / 60).toString().padStart(2, '0'); let s = (currentSecs % 60).toString().padStart(2, '0'); display.innerText = `${m}:${s}`; }, 1000);
    }
};
...
window.resetExTimer = function(exId) {
    ...
    window.exerciseTimers[exId] = { interval: null }; window.clearExTimerState(exId);
    document.getElementById(`display-extimer-${exId}`).innerText = "00:00";
    const btn = document.getElementById(`btn-extimer-${exId}`);
    btn.innerHTML = window.SVG_PLAY; btn.className = `flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-green-500 hover:bg-[#262626] transition-colors`;
};

window.resumeExTimerIfActive = function(exId) {
    let state = window.getExTimerState(exId); if (!state) return;
    let currentSecs = window.getExCurrentSeconds(exId);
    if (currentSecs > 0) {
        if(!window.exerciseTimers[exId]) window.exerciseTimers[exId] = { interval: null };
        ...
        if (state.running) {
            btn.innerHTML = window.SVG_PAUSE; btn.className = `flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-orange-500 hover:bg-[#262626] transition-colors`;
            t.interval = setInterval(() => { let curr = window.getExCurrentSeconds(exId); let mins = Math.floor(curr / 60).toString().padStart(2, '0'); let secs = (curr % 60).toString().padStart(2, '0'); if(display) display.innerText = `${mins}:${secs}`; }, 1000);
        } else {
            btn.innerHTML = window.SVG_PLAY; btn.className = `flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-green-500 hover:bg-[#262626] transition-colors`;
        }
    }
...
window.toggleTimer = function() {
    ...
    if (timerInterval) {
        ...
        timerInterval = setInterval(() => {
            ...
            if(timerSecondsLeft <= 10 && timerSecondsLeft > 0) window.playTap();
            if(timerSecondsLeft <= 0) { window.stopTimer(); window.playAlarm(); if (navigator.vibrate) navigator.vibrate([500, 200, 500]); window.openModal('modal-timer-finished'); } 
            else { window.updateTimerDisplay(); }
        }, 1000);
    }
};
...
window.resumeRestTimerIfActive = function() {
    ...
    if (savedEnd) {
        ...
        if (remaining > 0) {
            ...
            localStorage.removeItem('hat_rest_timer_end'); window.openModal('modal-timer-finished');
        }
    }
};

window.hideMiniTimer = function(stopAsWell = false) { const miniWidget = document.getElementById('mini-timer-widget'); if(miniWidget) { miniWidget.classList.add('translate-x-[-150%]', 'opacity-0'); miniWidget.classList.remove('translate-x-0', 'opacity-100'); } if (stopAsWell) window.stopTimer(); };
window.updateTimerDisplay = function() { let m = Math.floor(timerSecondsLeft / 60).toString().padStart(2, '0'); let s = (timerSecondsLeft % 60).toString().padStart(2, '0'); let txt = `${m}:${s}`; document.getElementById('timer-countdown').innerText = txt; if(document.getElementById('mini-timer-countdown')) document.getElementById('mini-timer-countdown').innerText = txt; };
...
window.handleAuth = async function() {
    ...
    if(!email || !password) return window.showMessage("Completá tu email y contraseña.", true); 
    btn.innerText = "Procesando..."; btn.disabled = true;
    try {
        if (isSignUp) { 
            const { data, error } = await supabaseClient.auth.signUp({ email, password, options: { emailRedirectTo: REDIRECT_URL } }); 
            if (error) throw error; 
            // Control contra "enumeración silenciosa" de Supabase
            if (data && data.user && data.user.identities && data.user.identities.length === 0) { throw new Error("Este email ya tiene cuenta registrada."); }
            window.showMessage("¡Cuenta creada con éxito! Ya podés iniciar sesión.", false); 
        } 
        else { 
            const { data, error } = await supabaseClient.auth.signInWithPassword({ email, password }); 
            if (error) throw error; currentUserId = data.user.id; window.loadDashboardView(data.user.email); 
        }
    } catch (err) { 
        let errorTxt = err.message; 
        if (errorTxt.includes("Invalid login credentials")) errorTxt = "❌ Email o contraseña incorrectos."; 
        if (errorTxt.includes("ya tiene cuenta")) errorTxt = "❌ Este correo ya está registrado en HAT."; 
        window.showMessage(errorTxt, true); 
    } 
    finally { btn.innerText = isSignUp ? "Registrarme" : "Iniciar Sesión"; btn.disabled = false; }
};

window.handleResetPassword = async function() {
    const email = document.getElementById('auth-email').value; if(!email) return window.showMessage("Ingresá tu email arriba y presioná este botón.", true);
    const btn = document.getElementById('btn-auth-action'); btn.innerText = "Enviando..."; btn.disabled = true;
    try { const { error } = await supabaseClient.auth.resetPasswordForEmail(email, { redirectTo: REDIRECT_URL }); if (error) throw error; window.showMessage("Si el email está registrado, te llegará un link para cambiar la contraseña.", false);
    } catch (err) { window.showMessage("Error: " + err.message, true); } finally { btn.innerText = "Iniciar Sesión"; btn.disabled = false; }
};
...
window.saveNewPassword = async function() {
    ...
    try {
        ...
        window.location.hash = ''; window.closeAllModals(); const { data: { session } } = await supabaseClient.auth.getSession();
        if(session) { currentUserId = session.user.id; window.loadDashboardView(session.user.email); }
    } catch(e) {
...
window.loadDashboardView = function(email) {
    try {
        ...
        window.closeAllModals(); window.updateCreditsDisplay(); 
        
        try { window.resumeGlobalWorkoutIfActive(); } catch(e) {}
        try { window.resumeRestTimerIfActive(); } catch(e) {}
        
        setTimeout(() => { window.changeDay('lunes'); }, 100);
    } catch(err) { console.error("Error al cargar dashboard", err); }
};

window.handleSignOut = async function() { if (supabaseClient) await supabaseClient.auth.signOut(); location.reload(); };

window.animateTokenLoss = function() {
...
window.updateCreditsDisplay = async function() {
...
window.openGlobalStats = async function() {
    window.openModal('modal-global-stats');
...
window.promptDeleteSession = function(id) {
    pendingDeleteSessionId = id; window.closeAllModals(); window.openModal('modal-confirm-delete-session');
    document.getElementById('btn-confirm-delete-session').onclick = async () => {
        ...
        try { await supabaseClient.from('workout_sessions').delete().eq('id', pendingDeleteSessionId); window.openGlobalStats(); } catch(e) { window.showToast("Error: " + e.message); } finally { btn.innerText = "Sí, Borrar"; btn.disabled = false; }
    };
};

// --- RUTINAS GUARDADAS ---
window.openRoutinesModal = async function() {
    window.openModal('modal-routines'); const list = document.getElementById('saved-routines-list'); list.innerHTML = '<div class="text-center text-xs text-custom-textMuted py-4">Buscando rutinas...</div>';
    try {
        ...
        let html = '';
        data.forEach(r => { html += `<div class="flex items-center justify-between bg-[#171717] p-3 rounded-xl border border-[#262626]"><span class="text-sm font-bold text-white">${escapeHTML(r.routine_name)}</span><div class="flex gap-2"><button onclick="window.promptLoadRoutine('${r.id}')" class="text-xs bg-custom-primary text-white px-3 py-1.5 rounded-lg font-bold hover:opacity-80">Cargar</button><button onclick="window.promptDeleteSavedRoutine('${r.id}')" class="text-xs border border-red-500/30 text-red-500 hover:bg-red-500 hover:text-white px-2 py-1.5 rounded-lg transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button></div></div>`; });
        list.innerHTML = html;
    } catch(e) { list.innerHTML = `<div class="text-xs text-red-500">Error: ${e.message}</div>`; }
};
window.promptSaveRoutine = function() { window.closeAllModals(); document.getElementById('routine-save-name').value = ''; window.openModal('modal-save-routine'); };
window.saveRoutine = async function() {
    const name = document.getElementById('routine-save-name').value.trim(); if(!name) { window.showToast("Escribí un nombre para la rutina."); return; }
    const { data: exercises, error } = await supabaseClient.from('user_routines').select('*').eq('user_id', currentUserId); if(error) { window.showToast("Error al leer rutina: " + error.message); return; }
    if(exercises.length === 0) { window.showToast("No tenés ejercicios para guardar."); return; }
    try { const { error: saveError } = await supabaseClient.from('saved_routines').insert([{ user_id: currentUserId, routine_name: name, routine_data: exercises }]); if(saveError) throw saveError; window.showToast("¡Rutina Guardada!"); window.openRoutinesModal(); } catch(e) { window.showToast("Error al guardar: " + e.message); }
};
window.promptLoadRoutine = function(savedId) {
    pendingSavedRoutineId = savedId; window.closeAllModals(); window.openModal('modal-confirm-load-saved');
    document.getElementById('btn-confirm-load-action').onclick = async () => {
        ...
        try {
            ...
            window.closeAllModals(); window.showToast("¡Rutina Cargada Exitosamente!"); window.changeDay(currentActiveDay);
        } catch(e) { window.showToast("Error al cargar: " + e.message); } finally { btn.innerText = "Sí, Cargar"; btn.disabled = false; }
    };
};
window.promptDeleteSavedRoutine = function(savedId) {
    pendingSavedRoutineId = savedId; window.closeAllModals(); window.openModal('modal-confirm-delete-saved');
    document.getElementById('btn-confirm-delete-saved-action').onclick = async () => {
        ...
        try { await supabaseClient.from('saved_routines').delete().eq('id', pendingSavedRoutineId); window.openRoutinesModal(); } catch(e) { window.showToast("Error: " + e.message); } finally { btn.innerText = "Sí, Borrar"; btn.disabled = false; }
    };
};
...
window.proceedWithAIGeneration = async function() {
    ...
    try {
        ...
        if(data.remaining_credits !== undefined) { const hasInf = document.getElementById('ai-credit-count').innerHTML.includes('infin'); if(!hasInf) { document.getElementById('ai-credit-count').innerText = data.remaining_credits; window.animateTokenLoss(); } }
...
window.analyzeProgress = function(exId, exName, exType) {
    ...
    if (dates.length < 3) { window.showToast("⚠️ Entrená al menos 3 veces para activar el análisis IA."); return; }
...
window.toggleMediaInput = function(type) { const checkbox = document.getElementById(`check-${type}`); const container = document.getElementById(`input-${type}-container`); checkbox.checked = !checkbox.checked; if(checkbox.checked) { container.classList.remove('hidden'); } else { container.classList.add('hidden'); document.getElementById(`new-ex-${type}`).value = ''; } };
...
window.changeDay = async function(day, event) {
    ...
    try {
        ...
        exercises.forEach(ex => {
            ...
            let state = window.getExTimerState(safeId); let currentExSecs = window.getExCurrentSeconds(safeId);
                ...
                <div class="flex flex-col items-center justify-center mt-4 mb-2 bg-[#0a0a0a] rounded-xl p-3 border border-[#262626]">
                    ...
                    <div class="flex items-center justify-center gap-4 w-full">
                        ...
                        <div class="flex gap-2">
                            ...
                            <button onclick="window.toggleExTimer('${safeId}')" id="btn-extimer-${safeId}" class="flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl ${tColorClass} hover:bg-[#262626] transition-colors" title="Play/Pausa">
                                ${tIcon}
                            </button>
                        </div>
                    </div>
        ...
        exercises.forEach(ex => window.resumeExTimerIfActive(ex.id));
...
window.promptDeleteExercise = function(exId, exName) { 
    ...
    btn.onclick = async () => { 
        ...
        try { const {error} = await supabaseClient.from('user_routines').delete().eq('id', exId); if(error) throw error; window.closeAllModals(); window.changeDay(currentActiveDay); } 
...
window.promptCopyExercise = function(exId) { exerciseToCopy = window.currentDayExercises.find(e => e.id === exId); document.getElementById('copy-target-day').value = currentActiveDay; window.openModal('modal-copy-exercise'); };
...
window.confirmCopyExercise = async function() {
    ...
    try { const {error} = await supabaseClient.from('user_routines').insert([newEx]); if(error) throw error; window.closeAllModals(); window.showToast("¡Ejercicio copiado exitosamente!"); if(targetDay === currentActiveDay) { window.changeDay(currentActiveDay); } } catch(e) { alert("Error al copiar: " + e.message); } finally { btn.innerText = "Copiar"; btn.disabled = false; }
};
...
window.saveToCloud = async function(exId, totalSets, exName, exType, btnEvent) {
    ...
    let exTotalSeconds = window.getExCurrentSeconds(exId);
    ...
    if(logs.length === 0) { const originalText = btnText.innerText; btn.classList.add('bg-red-600'); btnText.innerText = "MARCÁ 1 SERIE MÍNIMO"; setTimeout(() => { btn.classList.remove('bg-red-600'); btnText.innerText = originalText; }, 2000); return; } 
    ...
    try { 
        ...
        const evoContainer = document.getElementById(`evo-container-${exId}`); if(!evoContainer.classList.contains('hidden')) { window.loadEvolucion(exId, exName, exType, true); } 
        setTimeout(() => { btn.classList.replace('bg-green-600', 'bg-custom-primary'); btnText.innerText = "GUARDAR SESIÓN"; }, 2000); window.showToast("¡Entrenamiento registrado!"); window.playVictory(); 
...
window.loadEvolucion = async function(exId, exName, exType, forceReload = false) {
    ...
    try { 
        ...
        window.currentHistory[safeExId] = groupedData; const dates = Object.keys(groupedData); const chartDataMax = dates.map(d => groupedData[d].maxStat); const chartDataAvg = dates.map(d => Math.round((groupedData[d].totalStat / groupedData[d].totalSets) * 10) / 10); const chartDataDuration = dates.map(d => parseFloat((groupedData[d].duration / 60).toFixed(2)));
        ...
        <div>
            ...
            <div class="overflow-x-auto rounded-xl border border-custom-border bg-[#0a0a0a]">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-[#171717]"><tr class="border-b border-custom-border text-custom-textMuted text-[10px] uppercase tracking-widest"><th class="py-3 px-3 font-bold">Día</th><th class="py-3 px-3 font-bold text-center">Tiempo</th><th class="py-3 px-2 font-bold">Series</th><th class="py-3 px-3 font-bold text-right">Acción</th></tr></thead>
...
window.promptDeleteLog = function(exName, rawDate, exId, exType) {
    ...
    window.openModal('modal-confirm-delete'); const btn = document.getElementById('btn-confirm-delete'); const msgBox = document.getElementById('delete-error-msg'); 
    btn.onclick = async () => { 
        btn.innerText = "Borrando..."; btn.disabled = true; msgBox.classList.add('hidden'); 
        try { await supabaseClient.from('workout_logs').delete().eq('user_id', currentUserId).eq('exercise_name', exName).eq('log_date', rawDate); window.closeAllModals(); window.loadEvolucion(safeExId, exNameForJS, exType, true); } 

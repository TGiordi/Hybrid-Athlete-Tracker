...
// --- CRONÓMETRO DE DESCANSO GENERAL (Con persistencia offline) ---
...
function toggleTimer() {
    ...
    if (timerInterval) {
        ...
        // Guardamos el timestamp de cuándo tiene que terminar
        ...
        timerInterval = setInterval(() => {
            let endTime = parseInt(localStorage.getItem('hat_rest_timer_end'));
            timerSecondsLeft = Math.round((endTime - Date.now()) / 1000);
            ...
            if(timerSecondsLeft <= 0) { 
                ...
                openModal('modal-timer-finished');
            } else {
                updateTimerDisplay();
            }
        }, 1000);
...
function togglePasswordVisibility(inputId, iconId) { 
    const pwdInput = document.getElementById(inputId); const eyeIcon = document.getElementById(iconId); 
    if(pwdInput.type === 'password') { pwdInput.type = 'text'; eyeIcon.innerHTML = `<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>`; } 
    else { pwdInput.type = 'password'; eyeIcon.innerHTML = `<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"></path>`; } 
}
...
function loadDashboardView(email) {
    try {
        ...
        // Bloques try-catch aislados para que un error en el timer no rompa el login
        ...
        console.error("Error grave al cargar Dashboard:", err);
    }
}
                ...
                <div class="flex flex-col items-center justify-center mt-4 mb-2 bg-[#0a0a0a] rounded-xl p-3 border border-[#262626]">
                    ...
                    <div class="flex items-center justify-center gap-4 w-full">
                        ...
                        <div class="flex gap-2">
                            <button onclick="resetExTimer('${safeId}')" class="flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-red-500 hover:bg-red-500/10 transition-colors" title="Limpiar y Reiniciar a Cero">
                                ${SVG_RESET}
                            </button>
                            <button onclick="toggleExTimer('${safeId}')" id="btn-extimer-${safeId}" class="flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl ${tColor} hover:bg-[#262626] transition-colors" title="Play/Pausa">
                                ${tIcon}
                            </button>
                        </div>
                    </div>
...
async function saveExercise() {
    ...
    try { 
        if(currentEditExerciseId) { 
            ...
            exData.order_index = currentExercisesInDay; // Solo para nuevos
...
function promptDeleteLog(exName, rawDate, exId, exType) {
    ...
    btn.onclick = async () => { 
        ...
        try { 
            ...
            closeAllModals(); 
            loadEvolucion(safeExId, exNameForJS, exType, true); 
        } catch(e) { 
...
function promptEditLog(exId, exName, dateStr, exType) {
    ...
    btn.onclick = async () => { 
        ...
        finally { btn.innerText = "Actualizar Datos"; btn.disabled = false; } 
    }; 
    openModal('modal-edit-log');
}
// =========================================================================
// PUENTE DE FUNCIONES GLOBALES (DESBLOQUEO DE BOTONES HTML)
// =========================================================================

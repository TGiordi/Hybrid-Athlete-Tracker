...
function timerDragStart(e) {
    if(e.target.closest('button')) return; isDraggingTimer = true;
...
function timerDragEnd(e) {
    ...
    if(rect.top < 80) miniTimer.style.top = '80px'; if(rect.top > window.innerHeight - 100) miniTimer.style.top = `${window.innerHeight - 100}px`;
}
...
function toggleFabMenu() {
    ...
    if (options.classList.contains('opacity-0')) { options.classList.remove('opacity-0', 'translate-y-4'); options.classList.add('pointer-events-auto'); icon.style.transform = 'rotate(45deg)'; } 
    else { options.classList.add('opacity-0', 'translate-y-4'); options.classList.remove('pointer-events-auto'); icon.style.transform = 'rotate(0deg)'; }
}
window.closeFabAndRun = function(callback) { toggleFabMenu(); callback(); };
...
window.startGlobalWorkout = function() {
    ...
    globalTimerInterval = setInterval(() => { 
        const startTime = parseInt(localStorage.getItem('hat_workout_start') || Date.now()); globalSeconds = Math.floor((Date.now() - startTime) / 1000); 
        if (globalSeconds >= MAX_TIMER_SECONDS) { globalSeconds = MAX_TIMER_SECONDS; showToast("⚠️ Límite de 24h alcanzado."); } updateGlobalTimerDisplay(); 
    }, 1000);
};
...
function getExCurrentSeconds(exId) {
    let state = getExTimerState(exId); let secs = state.acc;
    if (state.running) { secs += Math.floor((Date.now() - state.lastStart) / 1000); }
    if (secs > MAX_TIMER_SECONDS) secs = MAX_TIMER_SECONDS; return secs;
}
...
window.toggleExTimer = function(exId) {
    ...
    if(state.running) {
        ...
        btn.innerHTML = SVG_PLAY; btn.classList.replace('text-green-400', 'text-orange-400'); if(btn.classList.contains('text-green-400')) btn.classList.replace('text-green-400', 'text-orange-400');
    } else {
        state.running = true; state.lastStart = Date.now(); saveExTimerState(exId, state);
        btn.innerHTML = SVG_PAUSE; btn.classList.replace('text-orange-400', 'text-green-400'); if(!btn.classList.contains('text-green-400')) btn.classList.add('text-green-400');
        t.interval = setInterval(() => {
            let currentSecs = getExCurrentSeconds(exId); if (currentSecs >= MAX_TIMER_SECONDS) currentSecs = MAX_TIMER_SECONDS;
            let m = Math.floor(currentSecs / 60).toString().padStart(2, '0'); let s = (currentSecs % 60).toString().padStart(2, '0');
...
window.hideMiniTimer = function(stopAsWell = false) { const miniWidget = document.getElementById('mini-timer-widget'); miniWidget.classList.add('translate-x-[-150%]', 'opacity-0'); miniWidget.classList.remove('translate-x-0', 'opacity-100'); if (stopAsWell) stopTimer(); };
...
window.toggleAuthMode = function() { isSignUp = !isSignUp; updateAuthUI(); };
...
window.promptSaveRoutine = function() { closeAllModals(); document.getElementById('routine-save-name').value = ''; openModal('modal-save-routine'); };
...
// --- COACH IA ---
window.handleAIGenerationRequest = function() { document.getElementById('ai-prompt').value = ""; openModal('modal-ai-coach'); };
...
window.openChatModal = function() { const savedChat = localStorage.getItem(`hat_chat_${currentUserId}`); if(savedChat) { window.chatHistory = JSON.parse(savedChat); } else { window.chatHistory = []; } renderChat(); openModal('modal-ai-chat'); };
window.promptClearChat = function() { closeAllModals(); openModal('modal-confirm-clear-chat'); };
window.cancelClearChat = function() { closeAllModals(); openModal('modal-ai-chat'); };
window.confirmClearChatHistory = function() { window.chatHistory = []; localStorage.removeItem(`hat_chat_${currentUserId}`); closeAllModals(); openChatModal(); };
function renderChat() {
...
window.changeDay = async function(day, event) {
    ...
    const container = document.getElementById('exercise-container'); container.innerHTML = '<div class="col-span-1 md:col-span-2 lg:col-span-3 xl:col-span-4 text-center text-custom-textMuted py-10 font-bold animate-pulse">Cargando tu rutina...</div>';
...
window.toggleExMenu = function(id) { const menu = document.getElementById(`ex-menu-${id}`); document.querySelectorAll('.ex-dropdown').forEach(el => { if (el.id !== `ex-menu-${id}`) el.classList.add('hidden'); }); menu.classList.toggle('hidden'); };
window.promptCopyExercise = function(exId) { exerciseToCopy = window.currentDayExercises.find(e => e.id === exId); document.getElementById('copy-target-day').value = currentActiveDay; openModal('modal-copy-exercise'); };
...
window.saveToCloud = async function(exId, totalSets, exName, exType, btnEvent) {
    ...
    try { 
        ...
        if(!evoContainer.classList.contains('hidden')) { loadEvolucion(exId, exName, exType, true); }
        setTimeout(() => { btn.classList.replace('bg-green-600', 'bg-custom-primary'); btnText.innerText = "GUARDAR SESIÓN"; }, 2000); 
        showToast("¡Entrenamiento registrado!"); playVictory(); 
...
window.promptDeleteLog = function(exName, rawDate, exId, exType) {
    ...
    btn.onclick = async () => { 
        ...
        try { await supabaseClient.from('workout_logs').delete().eq('user_id', currentUserId).eq('exercise_name', exName).eq('log_date', rawDate); closeAllModals(); loadEvolucion(safeExId, safeExName, exType, true); } 
...
window.promptEditLog = function(exId, exName, dateStr, exType) {
    const safeExId = escapeHTML(exId); const safeExName = escapeHTML(exName);
    const dayData = window.currentHistory[exId][dateStr]; document.getElementById('edit-log-title').innerText = `Corregir ${dateStr}`; 

...
async function loadEvolucion(exId, exName, exType, forceReload = false) {
    ...
    try { 
        ...
        window.currentHistory[safeExId] = groupedData; const dates = Object.keys(groupedData); const chartDataMax = dates.map(d => groupedData[d].maxStat); const chartDataAvg = dates.map(d => Math.round((groupedData[d].totalStat / groupedData[d].totalSets) * 10) / 10); const chartDataDuration = dates.map(d => Math.round(groupedData[d].duration / 60));

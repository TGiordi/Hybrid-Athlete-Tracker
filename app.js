        ...
        if (window.supabase) {
            ...
            if (isRecovering) { 
                ...
                if(session) { 
                    ...
                    window.loadDashboardView(session.user.email); 
                } 
            }
...
// DOBLE ENLACE DE FUNCIONES PARA EL BOTÓN FINALIZAR (Previene errores si el HTML usa otro nombre)
...
window.confirmStopGlobalWorkout = async function() {
    ...
    try {
        ...
        window.showToast("Error al guardar la sesión.");
    } finally {
...
window.toggleExTimer = function(exId) {
    ...
    if(state.running) {
        ...
        state.running = false;
        window.saveExTimerState(exId, state);
        
        btn.innerHTML = window.SVG_PLAY; 
        btn.className = `flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-green-500 hover:bg-[#262626] transition-colors`;
    } else {
        // Iniciar
        state.running = true;
        ...
        btn.className = `flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-orange-500 hover:bg-[#262626] transition-colors`;
...
window.resumeExTimerIfActive = function(exId) {
    ...
    if (currentSecs > 0) {
        ...
        if (state.running) {
            ...
            t.interval = setInterval(() => {
                let curr = window.getExCurrentSeconds(exId);
...
window.changeDay = async function(day, event) {
    ...
    try {
        ...
        exercises.forEach(ex => {
            ...
            let state = window.getExTimerState(safeId); 
            let currentExSecs = window.getExCurrentSeconds(safeId);
            let tDisplay = currentExSecs > 0 ? formatTime(currentExSecs) : "00:00";
                ...
                <div class="flex flex-col items-center justify-center mt-4 mb-2 bg-[#0a0a0a] rounded-xl p-3 border border-[#262626]">
                    ...
                    <div class="flex items-center justify-center gap-4 w-full">
                        ...
                        <div class="flex gap-2">
                            <button onclick="window.resetExTimer('${safeId}')" class="flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-red-500 hover:bg-[#262626] transition-colors" title="Limpiar y Reiniciar a Cero">
                                ${window.SVG_RESET}
                            </button>
...
window.saveToCloud = async function(exId, totalSets, exName, exType, btnEvent) {
    ...
    if(window.exerciseTimers[exId] && window.exerciseTimers[exId].interval) {
        ...
        state.running = false;
        window.saveExTimerState(exId, state);
        const btnPlay = document.getElementById(`btn-extimer-${exId}`);
        if(btnPlay) {
            btnPlay.innerHTML = window.SVG_PLAY; 
            btnPlay.className = `flex items-center justify-center w-12 h-12 bg-[#171717] border border-[#262626] rounded-xl text-green-500 hover:bg-[#262626] transition-colors`;
        }
    }
    ...
    try { 
        ...
        // Acá recién blanqueamos a cero porque la subida fue exitosa
        ...
        window.showToast("¡Entrenamiento registrado!"); 
        window.playVictory(); 
    } catch (e) { 
        btn.classList.add('bg-red-600'); 
...
window.loadEvolucion = async function(exId, exName, exType, forceReload = false) {
    ...
    try { 
        ...
        const chartDataDuration = dates.map(d => {
            // Pasamos a minutos con decimales para el gráfico
            let mins = groupedData[d].duration / 60;
        ...
        reversedDates.forEach(date => { 
            ...
            <tr class="border-b border-custom-border hover:bg-[#171717] transition-colors">
                ...
                <td class="py-3 px-2 text-xs text-white text-center font-bold whitespace-nowrap align-middle">${durText}</td>
...
window.promptEditLog = function(exId, exName, dateStr, exType) {
    ...
    // Calcular minutos y segundos desde los segundos totales
    ...
    btn.onclick = async () => { 
        ...
        try { 
            ...
            window.closeAllModals(); 
            window.loadEvolucion(safeExId, exNameForJS, exType, true); 
        } catch(e) { 
...
window.promptDeleteLog = function(exName, rawDate, exId, exType) {
    ...
    btn.onclick = async () => { 
        ...
        try { 
            ...
            msgBox.innerText = e.message; msgBox.classList.remove('hidden'); 
        } finally { 
            btn.innerText = "Borrar"; btn.disabled = false; 
        } 
    };
};
